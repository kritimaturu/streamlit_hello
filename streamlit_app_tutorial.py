...
def fetch_data():
    df = conn.query("SELECT tempF, humidity, YEAR(date_add(time_stamp,INTERVAL-5 "
                    "HOUR)) as year, MONTH(date_add(time_stamp,INTERVAL-5 HOUR)) as month, "
                    ...
                    "date_add(time_stamp,INTERVAL-5 HOUR) as ts FROM esp32_dht20 ORDER BY time_stamp "
                    'DESC LIMIT 5760;',ttl=1)
    return df
t=1
